...
<div style="display:flex;flex-direction:column;gap:14px">
  ...
  <!-- GW scores -->
    ...
    <div style="background:#111113;border-radius:20px;padding:22px 24px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
        ...
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <ExpandButton on:click={() => openModal = 'results'} />
        </div>
      </div>
    ...
    <Modal title="Gameweek {dashboard.gw} results" open={openModal === 'results'} on:close={() => openModal = null}>
  ...
  {#if dashboard.upcoming && dashboard.upcoming.fixtures.length > 0}
    <div style="background:#111113;border-radius:20px;padding:22px 24px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px">
        ...
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <ExpandButton on:click={() => openModal = 'upcoming'} />
        </div>
      </div>
    ...
    <Modal title="Upcoming fixtures" open={openModal === 'upcoming'} on:close={() => openModal = null}>
      <div style="position:relative">
        <div style="display:flex;flex-direction:column;gap:10px">
          {#each dashboard.upcoming.fixtures as fix (fix.key)}
            {#if fix.managers.length > 0}
              <div style="background:#17171a;border-radius:14px;padding:16px 18px">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;gap:12px">
                  ...
                  {#if fix.kickoff}
                    <span style="font:400 11.5px/1 Barlow,sans-serif;color:rgba(255,255,255,.35);white-space:nowrap;flex:none">{fix.kickoff}</span>
                  {/if}
                </div>
                <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:10px">
                  {#each fix.managers as mgr}
  ...
  <div style="background:#141416;border-radius:20px;padding:22px 24px">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;gap:12px">
      ...
      <div style="display:flex;align-items:center;gap:8px">
        ...
        <ExpandButton on:click={() => openModal = 'archive'} />
      </div>
    </div>
